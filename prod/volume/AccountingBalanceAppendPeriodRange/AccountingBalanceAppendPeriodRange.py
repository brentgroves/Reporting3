...
#%DEV%azure_dw = '1'
  ...
  start_period = rows[0][1] 
  end_period = rows[0][2] 

  cursor = conn.cursor()
  # accounting_balance_append_period_range_dw_import
  obj=cursor.execute("{call sproc300758_11728751_2000117 (?,?,?)}", pcn,start_period,end_period).rowcount
  rows = cursor.fetchall()

  print_to_stdout(f"call sproc300758_11728751_2000117 - len={len(rows)}")
